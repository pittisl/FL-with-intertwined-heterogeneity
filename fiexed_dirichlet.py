...
from utils import get_dataset, average_weights,get_dataset_dirichlet
import inversefed
...
# define pathsif  (np.sum(np.isin(top_10_client_index,idx))  )and epoch>delay-1 and dc!=0:
path_project = os.path.abspath('..')
logger = SummaryWriter('../logs')
device = 'cuda'
train_dataset, test_dataset, user_groups, top_10_client_index= get_dataset_dirichlet(args)
...
for epoch in tqdm(range(args.epochs)):
    ...
    for idx in idxs_users:
        ...
        if  (np.sum(np.isin(top_10_client_index,idx))  )and epoch>delay-1 and dc!=0:
