...
class GradientReconstructor():
    ...
    def reconstruct(self, input_data, labels, img_shape=(3, 32, 32), dryrun=False, eval=True, tol=None):
        ...
        return x_optimal.detach(), stats,labels
    ...
    def _run_trial(self, x_trial, input_data, labels, dryrun=False):
        ...
        if self.reconstruct_label:
            ...
            labels = torch.randn(output_test.shape).to(**self.setup).requires_grad_(True)
        ...
        try:
            for iteration in range(max_iterations):
                ...
                #print('x_trial',x_trial.shape)
                if self.config['lr_decay']:
                    scheduler.step()

                with torch.no_grad():
                    # Project into image space
                    #print('1',x_trial.shape)
                    if self.config['boxed']:
                        x_trial.data = torch.max(torch.min(x_trial, (1 - dm) / ds), -dm / ds)
                    #print(torch.min(x_trial, (1 - dm) / ds).shape)
...
def loss_steps(model, inputs, labels, loss_fn=torch.nn.CrossEntropyLoss(), lr=1e-4, local_steps=4, use_updates=True, batch_size=0):
    ...
    #for key, value in patched_model.parameters.items():
    #    print(key)
    #    print(value.shape)
    #print(len(patched_model.parameters))
    return list(patched_model.parameters.values())
    #return patched_model.parameters
def loss_steps_od(model, inputs, labels, loss_fn=torch.nn.CrossEntropyLoss(), lr=1e-4, local_steps=4, use_updates=True, batch_size=0):
    """Take a few gradient descent steps to fit the model to the given input."""
    patched_model = MetaMonkey(model)
    if use_updates:
        patched_model_origin = deepcopy(patched_model)
